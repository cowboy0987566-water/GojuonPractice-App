  ...
  return (
    <div className="flex flex-col flex-grow">
      <div className="flex justify-between items-center bg-white p-4 rounded-2xl shadow-sm border border-slate-100 mb-4">
        ...
        <div className="font-bold text-slate-700 text-lg">{calMonth.getFullYear()} - {String(calMonth.getMonth() + 1).padStart(2, '0')}</div>
      ...
      {selDateStr && dailyStats[selDateStr] && (
        <div className="bg-white p-5 rounded-2xl shadow-sm border-2 border-rose-100 relative overflow-hidden flex flex-col">
          ...
          <h3 className="font-bold text-slate-700 mb-4 text-sm flex items-center gap-2"><CalendarDays size={18} />{selDateStr}</h3>
          
          <div className="flex justify-between gap-3 text-center">
            <div className="bg-slate-50 p-2 rounded-xl flex-1"><DT tKey="tot" settings={settings} spanClass="text-[0.65rem] text-slate-500"/><div className="text-xl font-black">{dailyStats[selDateStr].total}</div></div>
          ...
          {dailyStats[selDateStr].wrongChars?.length > 0 && (
            <div className="mt-4 pt-4 border-t border-rose-100 flex flex-wrap gap-2">
              {dailyStats[selDateStr].wrongChars.map(romaji => {
                ...
                if (!kana) return null;
                return (
                  <button key={romaji} onClick={() => playAudio(kana.katakana)} className="bg-red-50 border border-red-200 px-2 py-1 rounded-lg text-sm font-bold text-slate-700">
